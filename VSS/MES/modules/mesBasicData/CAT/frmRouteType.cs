...
{
    public partial class frmRouteType : Form
    {
        public frmRouteType()
        {
            InitializeComponent();
        ...
        private void frmRouteType_Load(object sender, EventArgs e)
        {
            executeQuery();
        }
        ...
        void executeQuery()
        {
            foreach (string s in idv.mesCore.misc.RouteTypeGet())
            {
        ...
        void executeAdd()
        {
            if (!appInstance.CheckInputData(txtRouteType, lblRouteType)) return;
            if (!messageBox.showMessageById("msgConfirmExecute", messageStyle.askYesNo, cultureLanguage.getValue("add"))) return;
            try
            {
                idv.mesCore.misc.RouteTypeAdd(txtRouteType.Text, mesRelease.USR.User.loginUser.name);
                appInstance.showInformationById("msgExecuteSucceed", informationType.succeed);
                listView1.Items.Add(txtRouteType.Text).EnsureVisible();
                txtRouteType.Text = "";
        ...
        void executeDelete()
        {
            ...
            try
            {
                ...
                idv.mesCore.misc.RouteTypeDelete(item.Text);
                appInstance.showInformationById("msgExecuteSucceed", informationType.succeed);
