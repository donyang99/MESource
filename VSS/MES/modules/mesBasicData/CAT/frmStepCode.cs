...
{
    public partial class frmStepCode : Form
    {
        public frmStepCode()
        {
            InitializeComponent();            
        ...
        private void frmStepCode_Load(object sender, EventArgs e)
        {
            executeQuery();
        }
        ...
        void executeQuery()
        {
            foreach (string s in idv.mesCore.misc.StepCodeGet())
            {
        ...
        void executeAdd()
        {
            if (!appInstance.CheckInputData(txtStepCode, lblStepCode)) return;
            if (!messageBox.showMessageById("msgConfirmExecute", messageStyle.askYesNo, cultureLanguage.getValue("add"))) return;
            try
            {
                idv.mesCore.misc.StepCodeAdd(txtStepCode.Text, mesRelease.USR.User.loginUser.name);
                appInstance.showInformationById("msgExecuteSucceed", informationType.succeed);
                listView1.Items.Add(txtStepCode.Text).EnsureVisible();
                txtStepCode.Text = "";
        ...
        void executeDelete()
        {
            ...
            try
            {
                ...
                idv.mesCore.misc.StepCodeDelete(item.Text);
                appInstance.showInformationById("msgExecuteSucceed", informationType.succeed);
