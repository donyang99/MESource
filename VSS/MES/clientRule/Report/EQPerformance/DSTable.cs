    ...
    {        
        ...
        public string Render(System.Data.DataTable dt)
        {
            ...
            if (Accumulation)
            {
                ...
                //for EQPPerformance only
                ColumnSetting calcColumn = GetColumnSetting("Avail");
                calcColumn.AccSum = (GetColumnSetting("AVAILHOUR").AccSum / GetColumnSetting("TOTALHOUR").AccSum) * 100;
                calcColumn.tableCell.Text = calcColumn.AccSumFormat;

                calcColumn = GetColumnSetting("Non-Avail");
                calcColumn.AccSum = (GetColumnSetting("NON-AVAILHOUR").AccSum / GetColumnSetting("TOTALHOUR").AccSum) * 100;
                calcColumn.tableCell.Text = calcColumn.AccSumFormat;

                calcColumn = GetColumnSetting("Utilization");
                //calcColumn.AccSum = (GetColumnSetting("RUN").AccSum / GetColumnSetting("TOTALHOUR").AccSum) * 100;
                calcColumn.AccSum = (GetColumnSetting("RUN").AccSum / GetColumnSetting("AVAILHOUR").AccSum) * 100;
                calcColumn.tableCell.Text = calcColumn.AccSumFormat;
