...
namespace ClientRule.HoldLot
{
    public partial class frmMain : Form
    {
        ...
        void showLotStatus()
        {
            ...
            {
                if(item.Tag.ToString().Equals(idv.mesCore.WIP.LotStatus.WAIT.ToString()) ||
                    item.Tag.ToString().Equals(idv.mesCore.WIP.LotStatus.RUN.ToString()) ||
                    item.Tag.ToString().Equals(idv.mesCore.WIP.LotStatus.HOLD.ToString()))
                {
                    item.Checked = true;
                }
            }
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            mesRelease.WIP.Txn.HoldLot txn = new mesRelease.WIP.Txn.HoldLot();
        ...
        private void btnQuery_Click(object sender, EventArgs e)
        {
            ...
            if(!cboEquipmentType.Text.Equals("") && cboEquipmentId.Text.Trim().Equals(""))
            {
        ...
        private void lvwLots_MESItemCheckChanged(idv.messageService.itemBase item, ListViewItem listItem, bool selected, ref bool Cancel)
        {
            ...
            if (selected)
            {
                if(curReasonClass.Equals((item as Lot).stepId))
                    return;
                steps.Add((item as Lot).stepId);
            }
            foreach (Lot lot in lvwLots.selectedMESItems)
            {
                if(!selected && lot.name.Equals(item.name)) continue;
                if (!steps.Contains(lot.stepId))
                    steps.Add(lot.stepId);
            }
            if (steps.Count == 1)
            {
                if(curReasonClass.Equals(steps[0]))
                    return;
                else
                {
                    curReasonClass = steps[0];
                    reasonCode1.Init("Hold", curReasonClass);
                }
            }
            else
            {
                if(curReasonClass.Equals(""))
                    return;
                else
                {
                    curReasonClass = "";
                    reasonCode1.Init("Hold", "");
                }
            }
        }
