...
namespace NotifyWindow
{
    ...
    public class PopupNotifier : Component
    {
        ...
        public void Popup()
        {
            if (!disposed)
            {
                if (!frmPopup.Visible)
                {
                    ...
                    frmPopup.posIndex = GetAvailablePosIndex();
                    AddPosIndex(frmPopup.posIndex);
                    posStart -= frmPopup.posIndex * frmPopup.Height;
                    posStop -= frmPopup.posIndex * frmPopup.Height;
        ...
        public void Hide()
        {
            ...
            tmrAnimation.Stop();
            tmrWait.Stop();
            RemovePosIndex(frmPopup.posIndex);
            frmPopup.Hide();
        }
        ...
        private void tmAnimation_Tick(object sender, EventArgs e)
        {
            ...
            // animation has ended
            if (elapsed > AnimationDuration && Delay != 999)
            {
        ...
        static System.Collections.Generic.List<byte> posIndex = new System.Collections.Generic.List<byte>();
        public static byte GetAvailablePosIndex()
        {
            lock (posIndex)
            {
                for (byte i = 0; i < 5; i++)
                {
                    if (!posIndex.Contains(i))
                        return i;
                }
                return 0;
            }
        }
        public static void AddPosIndex(byte index)
        {
            lock (posIndex)
            {
                posIndex.Add(index);
        ...
        public static void RemovePosIndex(byte index)
        {
            lock (posIndex)
            {
                posIndex.Remove(index);
            }
        }

        public static PopupNotifier Notify(string title, string message)
        {
            return Notify(title, message, 0);
        }
        public static PopupNotifier Notify(string title, string message, int showSeconds)
        {
            return Notify(title, message, showSeconds, 300, 150);
        }
        public static PopupNotifier Notify(string title, string message, int showSeconds, bool warning)
        {
            Color color = Color.Teal;
            if (warning)
                color = Color.Red;
            return Notify(title, message, showSeconds, 300, 150, color);
        }   
        public static PopupNotifier Notify(string title, string message, int showSeconds, int width, int height)
        {
            return Notify(title, message, showSeconds, width, height, Color.Teal);
        }
        public static PopupNotifier Notify(string title, string message, int showSeconds, int width, int height, Color backColor)
        {
            PopupNotifier pn = new PopupNotifier();
            pn.Close += new EventHandler(pn_Close);
            pn.Click += new EventHandler(pn_Click);
            pn.TitleText = title;
            pn.ContentText = message;
            pn.ShowCloseButton = true;
            pn.ShowOptionsButton = false;
            pn.ShowGrip = true;
            pn.Delay = showSeconds * 1000;
            pn.AnimationInterval = 10;
            pn.AnimationDuration = 1000;
            pn.TitlePadding = new Padding(2);
            pn.ContentPadding = new Padding(2, 15, 2, 2);
            pn.ImagePadding = new Padding(2);
            pn.Scroll = true;
            pn.HeaderHeight = 16;
            pn.GradientPower = 300;
            if (width <= 0) width = 300;
            if (height <= 0) height = 150;
            pn.Size = new Size(width, height);
            pn.ContentFont = new Font(SystemFonts.CaptionFont.FontFamily, 12);
            pn.TitleFont = SystemFonts.MenuFont;
            pn.TitleColor = Color.DarkBlue;
            pn.BodyColor = backColor;//Color.Teal;
        ...
        public static event EventHandler PopupNotifierClose;
        public static event EventHandler PopupNotifierClick;
        static void pn_Click(object sender, EventArgs e)
        {
            if(PopupNotifierClick != null)
                PopupNotifierClick(sender, e);
        }

        static void pn_Close(object sender, EventArgs e)
        {
            if(PopupNotifierClose != null)
                PopupNotifierClose(sender, e);
        }
    }
}
